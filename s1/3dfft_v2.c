...
void transpose_xy_planes(fftwf_complex  tmp_fftw[][N][N], fftwf_complex in_fftw[][N][N])
{
    ...
    for (k=0; k<N; k++) {
     tareador_start_task("ffts1_transpose_xy_planes_loop_k");
     ...
     tareador_end_task("ffts1_transpose_xy_planes_loop_k");
...
int main (int argc, char *argv[])
{
  ...
  double stamp;
    ...
    tareador_start_task("ffts1_transpose_xy_planes");
    transpose_xy_planes(tmp_fftw, in_fftw);
    tareador_end_task("ffts1_transpose_xy_planes");

  
    ffts1_planes(p1d, tmp_fftw);
    

    tareador_start_task("ffts1_transpose_zx_planes");
    transpose_zx_planes(in_fftw, tmp_fftw);
    tareador_end_task("ffts1_transpose_zx_planes");

    
    ffts1_planes(p1d, in_fftw);
    

    tareador_start_task("ffts1_transpose_zx_planes2");
    transpose_zx_planes(tmp_fftw, in_fftw);
    tareador_end_task("ffts1_transpose_zx_planes2");
    
    tareador_start_task("ffts1_transpose_xy_planes2");
    transpose_xy_planes(in_fftw, tmp_fftw);
    tareador_end_task("ffts1_transpose_xy_planes2");
