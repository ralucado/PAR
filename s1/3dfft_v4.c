...
void init_complex_grid(fftwf_complex in_fftw[][N][N])
{
  ...
  for (k = 0; k < N; k++) {
    ...
    tareador_start_task("init_complex_grid_k_loop");
    for (j = 0; j < N; j++)
      for (i = 0; i < N; i++)
      {
    ...
    tareador_end_task("init_complex_grid_k_loop");
...
void transpose_zx_planes(fftwf_complex in_fftw[][N][N], fftwf_complex tmp_fftw[][N][N])
{
    ...
    for (k=0; k<N; k++) {
      tareador_start_task("ffts1_transpose_zx_planes_loop_k");
     ...
     for (j=0; j<N; j++)
      ...
      tareador_end_task("ffts1_transpose_zx_planes_loop_k");
